...
module.exports = function ({
    ...
    let height, self_height, start_height
    ...
    function complete(dir) {
        if (bot.player().getPos().y > start_height + .2) {
            const target_pos = start_height
            bot.item.select(tool, 0)
    ...
    return function (dir, offset) {
        self_height = 0
        start_height = bot.player().getPos().y
        ...
        bot.look.towards(bot.dir.SOUTHEAST, 0)
        bot.action.move(bot.dir.SOUTHEAST, 1)
        bot.action.move(bot.dir.NORTHWEST, 1)
    }
}
