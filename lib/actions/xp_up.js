module.exports = function (level=30, slot=0) {
    ...
    while (bot.player().getXPLevel() < level) {
        const xp_now = current_xp()
        const needed = xp_needed(level)
        if (needed >= 90 && has_emeralds()) {
            bot.item.ensure_min_count(emerald, 1, false)
            bot.item.select(emerald, slot)
            bot.item.unselect()
            bot.action.interact(bot.dir.get_dir(), 90, 100)
            bot.control.loop(() => xp_now === current_xp(), {timeout:250})
            continue
        }

        if (bot.item.count(xp_bottle) === 0){
            bot.item.ensure_min_count(emerald, 1, false)
            bot.item.ensure_min_count(xp_bottle, 1, false)
        }
        ...
        bot.control.loop(() => xp_now === current_xp(), {timeout:500})
    }
}
