...
module.exports = {
    ...
    toggle_sneak(value) {
        const has_sneak = this.key_held(this.SNEAK)
    ...
    key_held(key) {
        const bindings = KeyBind.getKeyBindings()
        const real_key = bindings.get(key) ?? key

        return KeyBind.getPressedKeys().contains(real_key)
            || (!bot.is_paused() && held_buttons.includes(real_key))
    },

    key_down(key) {
        const held = bot.input.key_held(key)
        const was_held = pressed_history.has(key)

        if (held) pressed_history.add(key)
        else pressed_history.delete(key)
