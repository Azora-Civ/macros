...
module.exports = {
    ...
    dig_down(height) {
        bot.look.towards(bot.dir.get_dir(false), 90)
        bot.input.add(bot.input.ATTACK)

        const target_y = bot.player().getPos().y - height;

        bot.control.loop(() => {
            let curr_height = bot.player().getPos().y
            return curr_height > (target_y + .2)
        })
...
function _sample(block= null) {
    ...
    bot.on_repeat.remove()

    bot.control.loop(() => item.is_holding(holding), {timeout:300, interval:25})
